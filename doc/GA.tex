...
Then with whatever version you are using open a terminal/command line in the \T{galebra} directory that is in the top directory
...
a latex distribution.  Directions follow if you do not already have {\LaTeX} installed.\footnote{Inorder for \T{galgebra} to output
...
the ansi console on windows is \emph{ansicon}.  The webpage for ansicon is \url{github.com/adoxa/ansicon}.  Since the authors of 
\emph{ansicon} allows it to be freely redistributed a copy of \T{ansicon.exe} has been included in the \emph{ansicon} directory of
this distribution.  In order to install \emph{ansicon} for console/terminal use run \T{echo \%PATH\%} from a terminal to display the
windows path an copy the appropriate version of \T{ansicon.exe} to the first directory listed in the path. The appropriate version of
\T{ansicon.exe} is in the directory \emph{ansicon\textbackslash x64} or \emph{ansicon\textbackslash x86}.  
If you are running 32-bit windows use the version
in subdirectory \emph{x86} otherwise use the one in subdirectory \emph{x64}.  Instructions for using \emph{ansicon} are given
in section~(\ref{stdprint}).\footnote{If you have nvidia graphics you may have to set the windows environment variable 
\T{ANSICON\_EXC=nvd3d9wrap.dll;nvd3d9wrapx.dll}.}
...
\begin{center}
   \begin{longtable}{ccc}
	...
	\end{longtable}
\end{center}
\vspace{-24pt}Line 5 of the previous listing illustrates the case of using the \T{mv} member function with
...
\begin{quote}
	...
	\end{longtable}	
	\end{center}
	\vspace{-36pt}\T{title} appends a title string to the beginning of the output.  An equal sign in
...
\begin{quote}
	Return the reflection of the mutivector $M$ in the subspace defined by the blade, \T{blade}, using \emph{Theorem 7.10} in 
...
For \T{Mv}, \T{Lt}, \T{Mlt}, and \T{Dop,} \T{Fmt()} returns a formatted string when executing from a script (\T{.py} file).  When executing in 
\emph{Ipython notebook} (\T{.ipynb} file) the function returns the \emph{object} itself.  For example if you wish to print a 
formatted multivector, \T{A},  from a script use

\T{    print A.Fmt(2,'A')}

for an \emph{Ipython notebook} session use
...
\bibitem {Macdonald 1} Alan Macdonald, ``Linear and Geometric Algebra,'' 2010. \url{http://faculty.luther.edu/~macdonal}
\bibitem {Macdonald 2} Alan Macdonald, ``Vector and Geometric Calculus,'' 2012. \url{http://faculty.luther.edu/~macdonal}
\end{thebibliography}
\end{document}
