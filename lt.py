...
def Smat(root,coords=None,mode='g'):
    return Symbolic_Matrix(root,coords=None,mode=mode,f=False,sub=True)
...
def Symbolic_Matrix(root,coords=None,mode='g',f=False,sub=True):
    ...
    if isinstance(root,list):
        ...
        if coords == None:
            raise ValueError('In Symbolic_Matrix coords = None" not allowed for root = '+str(root)+'\n')
        n = len(coords)
        n_range = range(n)
    ...
    if mode == 'g':  # General symbolic matrix
        ...
        if isinstance(root,list):
            for row in n_range:
                for col in n_range:
                    if row == col:
                        mat[row,col] = root[row][0]
                    elif row < col:
                        mat[row,col] = root[row][col-row]
                        mat[col,row] = root[row][col-row]
                    else:
                        pass
        else:
            for row in n_range:
    ...
    elif mode == 'a':  # Antisymmetric symbolic matrix
        if isinstance(root,list):
            for row in range(n-1):
                root[row] = [S(0)] + root[row]
            for row in n_range:
                for col in n_range:
                    if row < col:
                        mat[row,col] = root[row][col-row]
...
class Lt(object):
    ...
    def __init__(self, *kargs, **kwargs):
        ...
        if isinstance(mat_rep,list) and isinstance(mat_rep,list):  # Matrix list input
            if len(mat_rep) == n and len(mat_rep[-1]) == n:  # full square matrix input
                mat_rep = Symbolic_Matrix(mat_rep,mode='g')
            elif len(mat_rep) == n and len(mat_rep[-1]) == 1:  # symmetric matrix input
                mat_rep = Symbolic_Matrix(mat_rep,mode='s')
            elif len(mat_rep) == n-1 and len(mat_rep[-1]) == 1:  # asymmetric matrix input
                mat_rep = Symbolic_Matrix(mat_rep,mode='a')
            else:
                pass
        ...
        if isinstance(mat_rep, tuple):  # tuple input
            ...
            self.mat = mat_rep
            mat_rep = self.mat.T * self.Ga.g_inv
        ...
        if self.spinor:
            ...
            for base in self.Ga.basis:
                if base in self.lt_dict:
                    s += str(base) + '|-> ' + str(mv.Mv(self.lt_dict[base], ga=self.Ga)) + '\n'
                else:
                    s += str(base) + '|-> 0\n'
            return s[:-1]
    ...
    def Lt_latex_str(self):
        ...
        if self.spinor:
            ...
            for base in self.Ga.basis:
                s += str(base) + '\\mapsto &' + str(self.R * mv.Mv(base, ga=self.Ga) * self.Rrev) + ' \\\\ '
            ...
            for base in self.Ga.basis:
                if base in self.lt_dict:
                    s += str(base) + '\\mapsto &' + str(mv.Mv(self.lt_dict[base], ga=self.Ga)) + ' \\\\ '
                else:
                    s += str(base) + '\\mapsto &0 \\\\ '
            s = s[:-3] + ' \\end{array} \\right \\} \n'
            return s
