...
class Ga(metric.Metric):
    ...
    def pDiff(self, A: _mv.Mv, coord: Union[List, Symbol]) -> _mv.Mv:
        ...
        dA = self.mv(expand(diff(A if isinstance(A, Expr) else A.obj, coord)))
...
def Cl(p: int, q: int = 0, r: int = 0, root: str = 'e', **kwargs):
    r"""
    Construct a Clifford algebra :math:`Cl(p,q,r)` using a kingdon-like
    interface.
