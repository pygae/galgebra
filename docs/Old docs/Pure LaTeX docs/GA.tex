...
To install the geometric algebra module on windows, linux, or osx perform the following operations:
    ...
    \item If not already installed install python 2.6 or 2.7 from \url{http://python.org/} or on windows
	   ...
	   \begin{enumerate}
       \item To install \emph{sympy}
          \begin{enumerate}
           \item Go to \url{https://github.com/sympy/sympy}
           \item Download and unzip the repository zip file (Download ZIP button in window)
           \item Open a terminal (console) in the root directory of unzipped file
           \item Execute the command ``python setup.py install'' (``sudo'' not needed for windows)          
          \end{enumerate}
        \item The alternative install for windows is -
          \begin{enumerate}
           \item Get the latest \emph{sympy} binary from \url{http://code.google.com/p/sympy/downloads/list} for windows.
           \item Execute binary on your system to install \emph{sympy}.	 
           \end{enumerate}  
	   \end{enumerate}
    ...
    \item Go to \url{https://github.com/brombo/GA}
    \item Download and unzip the repository zip file (Download ZIP button in window)
    \item Open a terminal in the ga directory and execute the following command (``sudo'' not needed for windows) -
    \begin{enumerate}
    		\item ``sudo python setgapth.py'' for linux or OSX
    		\item ``python setgapth.py'' for windows
    \end{enumerate}
    \item It is strongly suggested that you go to \url{http://www.geany.org/Download/Releases} and install the version of the "geany" 
          editor appropriate for your system (there is a version of geany in macports for osx).
    \item If you wish to use \T{Eprint} on windows (this is not needed for linux or osx)-
        \begin{enumerate}
...
After installation for doing code development you need only include
...
in your python script to use the ga module.
...
\subsection{BAC-CAB Formulas}

This example demonstrates the most general constant metric tensor 
and how the GA module can be used to verify and expand geometric algebra identities consisting of relations between
the abstract vectors $a$, $b$, $c$, and $d$. The code and output are \href{./python/BACCAB.pdf}{baccab}.
...
basis vectors are correctly calculated.  The code and output are \href{./python/ReciprocalBasis.pdf}{reciprocal basis}.
...
and not as $E\W {2} =$.  The formulas derived for $E1$, $E2$, $E3$, and $E^{2}$ could
also be applied to the numerical calculations of crystal properties.

\section{Calculus}

\subsection{Derivatives in Spherical Coordinates}
...
$-I$ the negative of the pseudoscalar.  The code and output are \href{./python/SphericalCoordinates.pdf}{spherical coordinates}.
...
see \cite{Doran} chapter 7.  The example that follows is for linearly polarized
electromagnetic waves where $J=0$.  The code first shows that $\bm{E}$ and $\bm{B}$
must be orthogonal to $\bm{k}$ by the use of a general spatial metric. The the
orthogonality conditions are imposed on the fields and the orthogonality $\bm{E}$ and $\bm{B}$, 
the dispersion of the fields, and $\bm{B}$ in terms of $\bm{E}$ are calculated.  Note that
the calculation also employs the standard complex field representation in terms of exponents so
that in this case complex numbers and multivectors are mixed.  In the code $\T{I} = \sqrt{-1}$.
The code and output are \href{./python/EMWaves.pdf}{electromagnetic waves}.

\subsection{Dirac Equation}

In \cite{Doran} eq.~(8.89) (page 283) is the geometric algebra formulation of the Dirac equation.  In this equation
...
The code and output are \href{./python/Dirac.pdf}{dirac equation}.

\newpage\subsection{IPython Notebook Example}

Invoking Ipython with the command ``\T{ipython notebook --profile=sympy}'' we show a simple example of geometric algebra calculations in an IPython
notebook and how to print the latex representations of the multivectors.  Currently we must use the \T{Fmt()} member functions to print multivectors, differential operators, and linear transformations.  Other \emph{sympy} objects can be displayed in the normal way for Ipython notebook.
\begin{center}
\includegraphics[scale=0.4]{ipython/notebook.pdf}
\end{center}
Note that for the \T{Lt} and \T{Dop} classes the first argument of \T{Fmt} is a place holder.  For those classes all output is currently on one
line.  In the future that argument will be implemented so that long lines of output can be shortened.
