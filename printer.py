...
import re
import numbers
from sympy import Matrix, Basic, S, Symbol, Function, Derivative, Pow, Expr, Add
...
import printer
import builtins
from mv import Mv, Dop, Pdop, Sdop
...
def extract_fmt(s,delim):
    delim_indexes = find_all_char(s,delim)
    if len(delim_indexes) < 2:
        return s, ''
    i1 = delim_indexes[0]
    i2 = delim_indexes[1]
    fmt = s[i1+1:i2]
...
def rreplace(s, ch, sstr):
    i = s.rfind(ch)
    new_s = s[:i] + sstr + s[i+1:]
    return new_s
...
def err_print(s):
    if GaLatexPrinter.err is not None:
        GaLatexPrinter.err.write(s)
    return
...
class GaLatexPrinter(LatexPrinter):
    ...
    GaLatexPrinter.err =     title+' = '+str(A)
    ...
    def _print_Notes(self, expr):
        return expr.Notes_latex_str()
...
def lprint(expr):
    if isinstance(expr, (Expr, numbers.Number)):
        return(expr)
    latex_str = str(expr)
    if r'\begin{equation}' not in latex_str:
        latex_str = r'\begin{equation}' + latex_str + r'\end{equation}'
    return Latex(latex_str)

def Format(Fmode=True, Dmode=True, dop=1, inverse='full', debug=False):
    """
    ...
    if debug:
        GaLatexPrinter.err = file('err.txt','w')
    else:
        GaLatexPrinter.err = None
...
def xpdf(filename=None, paper=(14, 11), crop=False, png=False, prog=False, debug=False, pt='10pt'):
    ...
    if GaLatexPrinter.err is not None:
       GaLatexPrinter.err.close()
       os.system('geany err.txt')
...
def format_coef(coef,first,latex_flg=False):

    """
    first     : If true the coefficient is the first in an expression
    laxtex_flg: If ture use latex formatting
    """
    ...
    if isinstance(coef_obj, Add):  # Coefficient is sum of expressions
        ...
        if latex_flg:
            ...
            coef_str = '(' + coef_str + ')'
        if first and (sign == '+' or sign == ''):
            return coef_str
        elif not first:
            return sign + coef_str
        else:
            return coef_str
    else:
        if first:
